...
then
   ...
   conda init
   conda create --name issue_97125 python==3.11 pip -y
   eval "$(conda shell.bash hook)"
   conda activate issue_97125
   pip install -r requirements.txt
   pytest -sx
   returncode=$?
   conda deactivate
   conda env remove --name issue_97125 -y
...
docker build -t issue_97125 .
docker run -it --rm --gpus all issue_97125
exit $?