...
then
    ...
    conda init
    conda create --name issue_57498 python=3.9 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_57498
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_57498 -y
    exit ${returncode}
...
docker build -t issue_57498 .
docker run -it --rm --gpus all issue_57498
exit $?