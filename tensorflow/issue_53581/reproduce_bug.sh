...
then
    ...
    conda init
    conda create --name issue_53581 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_53581
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_53581 -y
...
docker build -t issue_53581 .
docker run -it --rm --gpus all issue_53581
exit $?