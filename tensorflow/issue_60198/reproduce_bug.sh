...
then
    ...
    conda init
    conda create --name issue_60198 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_60198
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_60198 -y
...
docker build -t issue_60198 .
docker run -it --rm --gpus all issue_60198
exit $?