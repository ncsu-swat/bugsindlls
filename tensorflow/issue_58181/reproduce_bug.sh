...
then
    ...
    conda init
    conda create --name issue_58181 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_58181
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_58181 -y
...
docker build -t issue_58181 .
docker run -it --rm --gpus all issue_58181
exit $?