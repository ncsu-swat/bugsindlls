...
then
    ...
    conda init
    conda create --name issue_57935 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_57935
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_57935 -y
...
docker build -t issue_57935 .
docker run -it --rm --gpus all issue_57935
exit $?