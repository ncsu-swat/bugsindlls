...
then
    ...
    conda init
    conda create --name issue_58178 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_58178
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_58178 -y
...
docker build -t issue_58178 .
docker run -it --rm --gpus all issue_58178
exit $?