...
then
    ...
    conda init
    conda create --name issue_60200 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_60200
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_60200 -y
...
docker build -t issue_60200 .
docker run -it --rm --gpus all issue_60200
exit $?