...
then
    ...
    conda init
    conda create --name issue_tfsa2022121 python==3.8 pip -y
    eval "$(conda shell.bash hook)"
    conda activate issue_tfsa2022121
    pip install -r requirements.txt
    pytest -sx
    returncode=$?
    conda deactivate
    conda env remove --name issue_tfsa2022121 -y
...
docker build -t issue_tfsa2022121 .
docker run -it --rm --gpus all issue_tfsa2022121
exit $?