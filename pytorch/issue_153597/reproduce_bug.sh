...
then
   ...
   conda init
   conda create --name issue_153597 python==3.11 pip -y
   eval "$(conda shell.bash hook)"
   conda activate issue_153597
   pip install -r requirements.txt
   pytest -sx
   returncode=$?
   conda deactivate
   conda env remove --name issue_153597 -y
...
docker build -t issue_153597 .
docker run -it --rm --gpus all issue_153597
exit $?