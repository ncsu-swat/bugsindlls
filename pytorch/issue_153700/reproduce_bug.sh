...
then
   ...
   conda init
   conda create --name issue_153700 python==3.11 pip -y
   eval "$(conda shell.bash hook)"
   conda activate issue_153700
   pip install -r requirements.txt
   pytest -sx
   returncode=$?
   conda deactivate
   conda env remove --name issue_153700 -y
...
docker build -t issue_153700 .
docker run -it --rm --gpus all issue_153700
exit $?