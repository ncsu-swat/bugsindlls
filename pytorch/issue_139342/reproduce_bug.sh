...
then
   ...
   conda init
   conda create --name issue_139342 python==3.11 pip -y
   eval "$(conda shell.bash hook)"
   conda activate issue_139342
   pip install -r requirements.txt
   pytest -sx
   returncode=$?
   conda deactivate
   conda env remove --name issue_139342 -y
...
docker build -t issue_139342 .
docker run -it --rm --gpus all issue_139342
exit $?