...
then
   ...
   conda init
   conda create --name issue_158172 python==3.11 pip -y
   eval "$(conda shell.bash hook)"
   conda activate issue_158172
   pip install -r requirements.txt
   pytest -sx
   returncode=$?
   conda deactivate
   conda env remove --name issue_158172 -y
...
docker build -t issue_158172 .
docker run -it --rm --gpus all issue_158172
exit $?