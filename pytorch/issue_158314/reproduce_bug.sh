...
then
   ...
   conda init
   conda create --name issue_158314 python==3.11 pip -y
   eval "$(conda shell.bash hook)"
   conda activate issue_158314
   pip install -r requirements.txt
   pytest -sx
   returncode=$?
   conda deactivate
   conda env remove --name issue_158314 -y
...
docker build -t issue_158314 .
docker run -it --rm --gpus all issue_158314
exit $?