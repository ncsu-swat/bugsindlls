...
\begin{table}
  ...
  \caption{\label{table:bug-stats}Different bug manifestation types
    from \tname.\Fix{If time allows, add a total column and sort rows
      according to it (except incorrect output)}}
  \begin{tabular}{lrrrr}\toprule
    Bug manifestation type &Jax &Pytorch &Tensorflow \\\midrule
    Incorrect Output &14 &11 &5 \\
    Internal Exception &11 &9 &4 \\
    Value Error &7 &0 &4 \\
    Type Error &3 &2 &3 \\
    Attribute Error &2 &0 &2 \\
    Index Error &2 &1 &0 \\
    Floating Point Error &1 &0 &0 \\
    Error Not Raised &1 &0 &0 \\
    Parse Error &1 &0 &0 \\
    Runtime Warning &1 &0 &0 \\
    Not Implemented Error &1 &0 &0 \\
    Memory Error &1 &1 &0 \\
    Invalid Argument Error &1 &0 &1 \\
    Runtime Error &0 &7 &1 \\
    Assertion Error &0 &2 &0 \\
    Segmentation Fault &0 &2 &1 \\
    Floating Point Exception &0 &1 &0 \\
    Aborted &0 &1 &8 \\
    \midrule
    $\Sigma$ &\numjaxbugs{} &\numtorchbugs{} &\numtfbugs{} \\
    \bottomrule
...
To run any tool on \tname, a script that contains commands to run the tool, a preprocessing script to extract error types and buggy APIs for a specific library version, and a postprocessing script to match the execution log with the expected errors needs to be provided. We provide implementations for FreeFuzz~\cite{wei2022free} in the \CodeIn{tool-integration} directory that contains templates for these scripts.

Tool execution proceeds in four steps: (1) Creation of Docker container
associated with a tool, (2) Reproduce an example bug from the library version provided as input (3) Running a preprocessing script to extract error types and buggy APIs for a specific library vesion (4) Execution of the tool and (5) Running a postprocessing script to match the execution log with the expected errors. Let us
...
The next four steps are executed with \tname's command \CodeIn{run-tool}.
When the comment is run, firstly an example bug that is reproducible with the library version provided as an input is run as a sanity check. Then, a preprocessing script to extract error types and buggy APIs for a specific library version runs.
...
three bugs that were expected to be reproduced since the three buggy APIs \CodeIn{torch.fake\_quantize\_per\_channel\_affine}, \CodeIn{torch.all}, and \CodeIn{torch.compile} are not supported by FreeFuzz. The outputs of the tool are saved in the container and can be inspected manually to further inspect the execution log.
