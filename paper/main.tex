...
follows. First, creation of docker container associated with the tool
done\Fix{ask gpt to revise/polish the text} with the script
\CodeIn{install\_freefuzz\_docker.sh}.  Then, an example bug\Fix{I
  don't understand ``example bug''. bugs in this version, no??!!} that
is reproducible with the library version provided as an input is run
as a sanity check.  Then, the preprocessing script
(\CodeIn{preprocess.py}) is executed to extract error types and buggy
APIs for a specific library vesion and the environment inside the
docker container of the tool is updated to match the library version
passed as an input and the fuzzing tool runs inside the
enviroment. After the execution is complete, the script finally runs a
postprocessing script (\CodeIn{postprocess.py}) to check the execution
log against the ground truth to determine if the bugs were reproduced
or not.
