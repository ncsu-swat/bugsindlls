...
\Fix{Clarify how the "evaluation set" used by the existing works is different from the "reference set" that we are proposing. Also, need more compelling reason/some sort of evaluation of other fuzzers
to complement the motivation}
...
this period. Details on tool availability is provided in
Section~\ref{sec:tool-availability}.
...
of instances for each kind of environment to reproduce the bug. Co-incidentally,
all bugs that require a GPU build are reproduced using a docker
container i.e. the column venv-docker is the same as bugs reproducible with CPU-GPU builds
of the libraries respectively. Table~\ref{table:bug-stats} shows whay type of error is manifested
due to each bug instance. In total, we have \numbugtypes\ different types of bug manifestations across
the three libraries.

\Fix{Added table and description, please revise}
...
\numtfbugs{}, and \numjaxbugs{} bug instances for \torch, \tf,
...
\begin{table}
  \centering
  \caption{\label{table:bug-stats}Different bug manifestation types from \tname.}
