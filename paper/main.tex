...
execution log with the expected errors needs to be provided.\Fix{name
  these scripts here and refer back to them in the text below} The
...
script to match the execution log with the expected errors.\Fix{up to here}
...
following instructions\Fix{add brief explanation for what it does}:
...
\CodeIn{run-tool}.\Fix{I don't know what are the four steps (I can't
  remember anymore)}

\Fix{Can't you just explain what is the input-output and what it does
  (at a very high level)?}When the command is run, firstly an example bug that is reproducible with the library version provided as an input is run as a sanity check. Then, a preprocessing script to extract error types and buggy APIs for a specific library version runs.
...
supported by FreeFuzz.\Fix{This is bad in two way: (1) we did not give
