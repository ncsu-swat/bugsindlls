...
\copyrightyear{2025}
\acmYear{2025}
\acmDOI{10.1145/3713081.3731739}
\acmConference[ISSTA Companion '25]{34th ACM SIGSOFT International Symposium on Software Testing and Analysis}{June 25--28, 2025}{Trondheim, Norway}
\acmBooktitle{34th ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA Companion '25), June 25--28, 2025, Trondheim, Norway}
\acmISBN{979-8-4007-1474-0/2025/06}
%%
%% Submission ID.
...
\affiliation{%
  \institution{NC State University, Raleigh, USA}
  % \city{Raleigh}
  % \country{USA}
  \country{}
}
...
\author{Benjamin Wu}
\affiliation{%
 \institution{Purdue University, West Lafayette, USA}
%  \city{West Lafayette}
%  \country{USA}
 \country{}
}
...
evaluation standard is a serious obstacle to a fair and rigorous
...
criteria:
% \vspace{-2ex}
\begin{enumerate}[leftmargin=0.2in,topsep=0pt]
\item ability to write test scripts in Python;
\item handlin nightly builds and specific
  CUDA versions (\textsection~\ref{sec:challenges});
\item ability to integrate fuzzing tools in the framework.
\end{enumerate}  
% \vspace{-2ex}
...
\tname\ is publicly available \cite{bugsindlls}. A video demonstrating
\tname\ is also available \cite{bugsindllsyoutube}.
...
GPU. Table~\ref{table:bug-stats} shows the breakdown of error types for
...
outputs of test runs on a CPU and on a GPU. In total, we have
...
bugs in the core API functionality, as well as some cases that are simply API misuses, not bugs. These
...
or can not be reproduced due to requiring a debug build that only the developers can access. We successfully reproduce the bug if none of these
...
virtual environments are not designed to enable changing OS drivers.
...
\begin{enumerate}[leftmargin=0.5cm]
  ...
  \item Write a script that creates the virtual environment (either
    Python's \CodeIn{Conda Enviroment}~\cite{venv} or Docker container~\cite{merkel2014docker}) and runs
    the test on it;
  \item Create a directory \CodeIn{<lib>/<bug-id>} containing all the
    artifacts mentioned above, where \CodeIn{lib} is the
    affected library and \CodeIn{bug-id} is the issue number of the bug report.
    % is a fresh identifier of the
    % bug instance.
...
%% number of tests that reproduce bugs in C or Python
%% code, etc.)\\
\bottomrule
\end{tabular}
  \vspace{-3ex}
...
execution log with the expected errors (\CodeIn{postprocess.py}).
% \Fix{name
%   these scripts here and refer back to them in the text below} 
The
directory \CodeIn{tool-integration} contains templates for these
...
\noindent
The script \CodeIn{install\_freefuzz\_docker.sh} builds the docker
...
2.2.1+cu121 of \torch.  The command \CodeIn{run-tool} takes the library
...
(\CodeIn{run\_freefuzz\_docker.sh}). The script reports how
...
% since the three buggy APIs \CodeIn{torch.fake\_quantize\_per\_channel\_affine}, \CodeIn{torch.all}, and \CodeIn{torch.compile} are not supported by FreeFuzz. 
Among the two buggy APIs, FreeFuzz supports \CodeIn{torch.logsumexp} but the bug could not be reproduced, whereas FreeFuzz does not support \CodeIn{torch.autograd.grad}.
The outputs from the tool are saved in the container so that the execution log can be inspected further manually.
% can be inspected manually to further inspect the execution log. 
